...
$$
q^* = \argmin_{q \in \mathcal{Q}} \KL(q\|p) = \argmin_{q \in \mathcal{Q}} \int q(\boldsymbol{\theta}) \log \frac{q(\boldsymbol{\theta})}{p(\boldsymbol{\theta}|D)} d\boldsymbol{\theta}
$$
...
2. <strong>While</strong> stopping criteria not met:
   1. <strong>Laplace Step</strong>:
      - Compute $$\hat{\boldsymbol{\eta}}^{(k)} = \argmax f(\boldsymbol{\eta})$$
