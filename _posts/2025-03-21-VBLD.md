...
\\[
q^* = \argmin_{q \in \mathcal{Q}} \text{KL}(q\|p) = \argmin_{q \in \mathcal{Q}} \int q(\boldsymbol{\theta}) \log \frac{q(\boldsymbol{\theta})}{p(\boldsymbol{\theta}|D)} d\boldsymbol{\theta}
\\]
...
<ul>
  <li><strong>Importance Sampling</strong>... but high variance in high dimensions</li>
...
<p>The Laplace approximation yields:</p>
$$
  ...
  <li><strong>While</strong> stopping criteria not met:
    <ol>
      <li><strong>Laplace Step</strong>:
        <ul>
          <li>Compute \\( \hat{\boldsymbol{\eta}}^{(k)} = \argmax f(\boldsymbol{\eta}) \\)</li>
